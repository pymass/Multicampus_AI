...
select cmf, PARTY_NM from card_tran_201311 where coalesce(inst_amt, 0) = 0;
-- 3. pb고객을 대상으로 일시불사용금액의 1%를 캐시백해주는 캠페인을 진행하면 누가 얼마의 금액을 받을 것인가
select PARTY_NM, (coalesce(pif_amt, 0)*0.01) as 캐시백 from card_tran_201311 where seg = 'pb';
-- 4. 고객 등급별 총사용 금액을 추출하고 총사용금액 기준 내림차순 정렬
select seg, sum(coalesce(pif_amt, 0) + coalesce(inst_amt, 0) + coalesce(ovrs_amt, 0) + coalesce(CASH_AMT, 0)) as 총사용금액 from card_tran_201311 group by seg order by 총사용금액 desc;
-- 5. 사용금액이 가장 많은 서비스는
...
select (purchase_amt / purchase_cnt) as 올해평균구매금액,
(last_amt / last_cnt) as 작년평균구매금액 from purchase_tran;
