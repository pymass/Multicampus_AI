...
select round(annl_perf, 0) from cust_info;
select format(annl_perf, 0) from cust_info;
...
select staff_nm, floor(datediff(now(), birth_dt)/365) as age from clerk;
select staff_nm, birth_dt, adddate(birth_dt, 30) as monthlater from clerk;
