...
-- 2. 캠페인 성공률을 계산하시오
select avg(case when balance_201312 > balance_201311*1.1 then 1 else 0 end) * 100 as 성공률 from casa_201312;
-- 3. 캠페인의 결과로 증가된 수신고 순 증가분을 구하시오
select (sum(balance_201312) - sum(balance_201311)) as 증가분 from casa_201312;
-- 4. 캠페인 결과 수신고 증가율을 구하시오 
select (sum(balance_201312)-sum(balance_201311))/sum(balance_201311) * 100 as 증가율 from casa_201312;
...
select count(music_score) from stud_score where music_score is not null;
...
select student_id, (coalesce(math_score,0)+ coalesce(eng_score,0)+coalesce(phil_score,0)+coalesce(music_score,0)) 
as score from stud_score order by score desc limit 1;
...
select job, current_sal, (case when eng_score >= 90 then current_sal*1.05 else current_sal end) as 연봉인상률 from staff_sal;

select id, job, current_sal, case 
when job = 'clerk' and eng_score > 90 then current_sal * 1.12
